...
int CullBack=1;    //flag for toggling culling
...
void myKeyboardFunc( unsigned char key, int x, int y )
{
	...
	case 'r':
		...
		break;
    case 'c':  
            CullBack=1-CullBack; //toggle the flag
            if(CullBack){
            ...
            glutPostRedisplay(); //so that you can see the animation taking effects right away
            break;
	case 27:	// Escape key
		exit(1);
	}
...
void mySpecialKeyFunc( int key, int x, int y )
{
	...
	glutPostRedisplay();  //pay attention to it
...
void myDrawColorShape1() {
	...
	glVertex3f( 0.0, 0.75*sqrt(2), 0.0 );               //v1--fan center
	glVertex3f( 0.0, -0.25*sqrt(2), 1.0 );              //v2
	glVertex3f( 0.5*(sqrt(3)), -0.25*sqrt(2), -0.5 );   //v3
 	

	glColor3f( 1.0, 0.0, 0.0 );		// Red
    
    glVertex3f(-0.5*sqrt(3), -0.25*sqrt(2), -0.5);      //v4
    ...
    glBegin(GL_TRIANGLES);      //draw the additional triangle at the bottom
    ...
    //NOTICE THE ORDER OF VERTICES ---CCW
...
void myDrawColorShape2() {
    ...
    glVertex3f( 0.0, 0.75*sqrt(2), 0.0 );               //v1-vertex
